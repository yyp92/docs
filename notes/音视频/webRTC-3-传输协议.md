...
- 一旦你启动抓包，这里会瞬间展示抓到的各种协议的大量数据包（下图展示wireshark每个区域的功能），其中在①过滤栏中输入UDP进行过滤，然后就会在②数据包列表中只展示出udp的数据包，并会解析出部分协议的数据包，并在Protocol列清楚的看到它们的协议。
